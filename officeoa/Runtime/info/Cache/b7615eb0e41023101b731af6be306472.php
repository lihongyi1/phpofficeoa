<?php if (!defined('THINK_PATH')) exit();?>			<!-- Messages Popup Content: Start -->
			<div class="box_content padding " id="messages" style="overflow: auto;height:400px">
					...
					<?php if(is_array($msgs)): $i = 0; $__LIST__ = $msgs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$msg): $mod = ($i % 2 );++$i;?><h6>
						<span class="icon user"></span> <a href="javascript:void(0)"><?php if($msg['fromid']['userid'] == $usersession['userid']): ?>我<?php else: echo ($msg['fromid']['realname']); endif; ?></a> <span class="nobold">说：</span>
					...
					<div class="field">
						...
						<textarea cols="30" rows="7" id="replyinbox"></textarea>
					</div>
				  
				  <button class="replyinbox">发送</button>
...
$('.replyinbox').click(function(){
	...
	$.post(
			"__APP__/Info/reply",
			{'rid':rid,'content':$('#replyinbox').val()},
			function(data){
					alert(data);
				})
})
